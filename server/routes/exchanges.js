...
const ENCRYPTED_FIELDS = ['account_email', 'password_hint', 'api_key_note', 'assets', 'notes', 'exchange_uid', 'deposit_addresses', 'referral_link', 'referral_code'];
...
router.post('/', authMiddleware, (req, res) => {
  try {
    const { profile_id, exchange_name, account_email, password_hint, kyc_status, api_key_note, assets, total_value_usd, screenshots, notes, exchange_uid, deposit_addresses, login_devices, referral_link, referral_code } = req.body;
    ...
    const encrypted = encryptExchange({ account_email, password_hint, api_key_note, assets, notes, exchange_uid, deposit_addresses, referral_link, referral_code }, getEncryptionKey(req));

    const result = db.prepare(`
      INSERT INTO exchanges (user_id, profile_id, exchange_name, account_email, password_hint, kyc_status, api_key_note, assets, total_value_usd, screenshots, notes, exchange_uid, deposit_addresses, login_devices, referral_link, referral_code)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      ...
      encrypted.referral_link,
      encrypted.referral_code
    );
...
router.put('/:id', authMiddleware, (req, res) => {
  try {
    ...
    const { exchange_name, account_email, password_hint, kyc_status, api_key_note, assets, total_value_usd, screenshots, notes, profile_id, exchange_uid, deposit_addresses, login_devices, referral_link, referral_code } = req.body;

    const encrypted = encryptExchange({ account_email, password_hint, api_key_note, assets, notes, exchange_uid, deposit_addresses, referral_link, referral_code }, getEncryptionKey(req));

    db.prepare(`
      UPDATE exchanges SET exchange_name=?, account_email=?, password_hint=?, kyc_status=?, api_key_note=?, assets=?, total_value_usd=?, screenshots=?, notes=?, profile_id=?, exchange_uid=?, deposit_addresses=?, login_devices=?, referral_link=?, referral_code=?, last_updated=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP
