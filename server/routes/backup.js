...
router.get('/export/csv', authMiddleware, (req, res) => {
  try {
    ...
    if (sectionList.includes('exchanges')) {
      ...
      csv += '=== 交易所列表 ===\n';
      if (showPasswordHints) {
        csv += '身份,交易所,UID,账户邮箱,密码提示,KYC状态,充值地址,登录设备,总估值USD,创建时间\n';
      } else {
        csv += '身份,交易所,UID,账户邮箱,KYC状态,充值地址,登录设备,总估值USD,创建时间\n';
      }
      csvExchanges.forEach(ex => {
        ...
        if (showPasswordHints) {
          csv += `"${ex.nickname}","${ex.exchange_name}","${uid}","${decryptField(ex.account_email, key) || ''}","${decryptField(ex.password_hint, key) || ''}","${ex.kyc_status}","${depositAddrs}","${devices}","${ex.total_value_usd || 0}","${ex.created_at}"\n`;
        } else {
          csv += `"${ex.nickname}","${ex.exchange_name}","${uid}","${decryptField(ex.account_email, key) || ''}","${ex.kyc_status}","${depositAddrs}","${devices}","${ex.total_value_usd || 0}","${ex.created_at}"\n`;
        }
      });
    }
        ...
        for (const ex of data.exchanges) {
          ...
          db.prepare(`
            INSERT INTO exchanges (user_id, profile_id, exchange_name, account_email, password_hint,
              kyc_status, api_key_note, assets, total_value_usd, notes, exchange_uid, deposit_addresses, login_devices)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).run(
            userId,
            newProfileId,
