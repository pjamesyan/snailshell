...
// GET /api/profiles/device-history - get all unique login devices used
router.get('/device-history', authMiddleware, (req, res) => {
  try {
    const devices = new Set();
    // Collect from profiles
    const profiles = db.prepare('SELECT login_devices FROM profiles WHERE user_id = ?').all(req.user.id);
    for (const p of profiles) {
      if (p.login_devices) {
        try { JSON.parse(p.login_devices).forEach(d => { if (d && d.trim()) devices.add(d.trim()); }); } catch {}
      }
    }
    // Collect from projects
    const projects = db.prepare('SELECT login_devices FROM projects WHERE user_id = ?').all(req.user.id);
    ...
    const exchanges = db.prepare('SELECT login_devices FROM exchanges WHERE user_id = ?').all(req.user.id);
    for (const e of exchanges) {
      if (e.login_devices) {
        try { JSON.parse(e.login_devices).forEach(d => { if (d && d.trim()) devices.add(d.trim()); }); } catch {}
      }
    }
    res.json({ devices: [...devices].sort() });
  } catch (err) {
    console.error('Device history error:', err);
    res.status(500).json({ error: '获取设备历史失败' });
  }
});
...
router.get('/', authMiddleware, (req, res) => {
  try {
    ...
    let profiles = db.prepare('SELECT * FROM profiles WHERE user_id = ? ORDER BY sort_order ASC, created_at ASC').all(req.user.id);
